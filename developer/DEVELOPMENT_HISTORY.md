...
## Version 0.65.4: Player Changelog Backfill and History Cleanup

**WHATS_NEW.md backfilled for players**

Releases 0.60.1 through 0.64.6 shipped without player changelog entries even
though 0.64.6 changed the instruction block and requires a re-paste. A
retroactive 0.64.6 entry now tells players to re-paste and summarizes the
container breakdown fix, the delivery crediting guardrail, and same-pickup
same-commodity conflict detection. A combined entry marks 0.60.1 through
0.64.5 as internal releases requiring no player action.

**Development history ordering fixed**

The version sections in this file had drifted into inconsistent order (0.58.x
after 0.38.1, recent sections newest-first while old sections were
oldest-first), and one header was duplicated ("Version 0.36.2: Player
Accessibility Release" is 0.36.1 per VERSION.json). The duplicate header was
corrected and all 63 version sections were reordered chronologically with a
script that asserts content preservation. New sections are appended at the end.

**Patch lineage registry now keeps a lineage**

patch_lineage_registry.json declared a patch_lineage field but only ever
overwrote current_patch in place, losing the trail. A patch_lineage array now
records Alpha 4.8.0 as superseded and Alpha 4.8.3 as current.

**Duplicate test fixtures differentiated**

sample_provider_output.json, sample_telemetry.json, and sample_valid.json were
byte-identical, which weakened their value as distinct fixtures. The telemetry
and provider output samples now contain semantically appropriate content;
sample_valid.json remains the generic valid input. The test checksum manifest
was regenerated and verified round-trip.
