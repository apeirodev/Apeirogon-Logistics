...
| AI-02 | CLAUDE.md, OWASP LLM02 | Sanitize provider outputs | `OCR_result_normalizer.py` strips injection-pattern text and flags `potential prompt-injection text detected`. | `developer/tools/OCR_result_normalizer.py: normalize_ocr()` |
...
| WARN-02 | CLAUDE.md | 10 MB input size cap | `safe_load_json()` checks `st_size` before reading; raises `ValueError` for files exceeding 10 MB. `bundle_telemetry.py` enforces `_MAX_FILE_BYTES` per file. | `developer/tools/lib/common.py: safe_load_json()`, `developer/tools/bundle_telemetry.py` |
...
| AI-02 (full) | CLAUDE.md | Sanitize provider outputs | Injection pattern check exists only in `normalize_ocr()`. The AI vision path (`_normalise_vision()`) does not check for injected instructions in free-text fields inside missions. | `developer/tools/OCR_result_normalizer.py: _normalise_vision()` |
...
| SC-02 | supply-chain-python.md | pip-audit in CI | The CI workflows do not run `pip-audit`. Manual audit confirms no current CVEs. | `.github/workflows/validation.yml` |
...
| SC-02: pip-audit not run in CI | Medium | Manual pip-audit confirms no current CVEs | Add `pip-audit` step to validation.yml |
...
| AI-02 (partial): Injection patterns not checked in AI vision free-text fields | Low | Vision path processes structured missions, not free text | Extend injection check to string fields within mission objects in `_normalise_vision()` |
